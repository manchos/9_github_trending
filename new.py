...
def output_repositories(repositories_namedtuple):
    for repo in repositories_namedtuple:
        # repo = repository_info_class(*repo)
        print(
            '\n\n owner: {repo.owner}'
            '\n name: {repo.name}'
            '\n url: {repo.url}'
            '\n\t\t Open {N} issues:'.format(repo=repo, N=len(repo.issues))
        )
        if len(repo.issues):
...
if __name__ == '__main__':
    ...
    if parser.parse_args().cache_time:
        print('parser.parse_args().cache_time')
        requests_cache.install_cache('cache/page_cache', backend='sqlite', expire_after=parser.parse_args().cache_time)
    else:
        requests_cache.install_cache('cache/page_cache', backend='sqlite')
